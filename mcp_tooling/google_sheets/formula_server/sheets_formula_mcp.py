...
app = FastAPI(title="Google Sheets Formula MCP Server", version="1.0.0")
...
async def health_check():
    ...
    return {"status": "healthy", "service": "Google Sheets Formula MCP Server"}
...
def main():
    ...
    port = 3012  # Default port for formula server
    if len(sys.argv) > 1:
        try:
            port = int(sys.argv[1])
        except ValueError:
    ...
    logger.info(f"Starting Google Sheets Formula MCP Server on port {port}")
